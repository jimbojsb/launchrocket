...
A Mac PreferencePane for managing services with `launchd`. For an introduction on `launchd`, see [launchd.info](http://launchd.info/). LaunchRocket was primarily created for managing various services installed by Homebrew, though it should work with most launchd-compatible plists.
